...
Node::SPtr Simulation::load ( const std::string& filename, const std::vector<std::string>& sceneArgs )
{
    ...
    if (loader) return loader->load(filename, sceneArgs);

    // unable to load file
    serr << "extension ("<<sofa::helper::system::SetDirectory::GetExtension(filename.c_str())<<") not handled" << sendl;
    return NULL;
}
